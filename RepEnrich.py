...
repeat_bed = setup_folder + '/repnames.bed' 
...
fin = import_text(setup_folder + '/' + 'repgenomes_key.txt', '\t')
repeat_key ={}
rev_repeat_key ={}
repeat_list = []
reptotalcounts = {}
...
fileout= outputfolder + '/' + outputfile_prefix + '_regionsorter.txt'
with open(fileout, 'w') as stdout:
   command = shlex.split("coverageBed -abam " +unique_mapper_bam+" -b " +setup_folder + '/repnames.bed')
   p = subprocess.Popen(command,stdout=stdout)
p.communicate()
stdout.close()
filein = open(outputfolder + '/' + outputfile_prefix + '_regionsorter.txt','r')
counts = {}
sumofrepeatreads=0
for line in filein:
	line= line.split('\t')
...
##############################################################################################
if paired_end == 'TRUE':
	if not os.path.exists(outputfolder + '/' + 'pair1_bowtie'):
		os.mkdir(outputfolder + '/' + 'pair1_bowtie')
	if not os.path.exists(outputfolder + '/' + 'pair2_bowtie'):
		os.mkdir(outputfolder + '/' + 'pair2_bowtie')
	folder_pair1 = outputfolder + '/' + 'pair1_bowtie'
	folder_pair2 = outputfolder + '/' + 'pair2_bowtie'
##############################################################################################
	...
	for metagenome in repeat_list:
   		metagenomepath = setup_folder + '/' + metagenome
		file1=folder_pair1 + '/' + metagenome + '.bowtie'
		file2 =folder_pair2 + '/' + metagenome + '.bowtie'
...
##############################################################################################
	...
	if not os.path.exists(outputfolder + '/' + 'sorted_bowtie'):
		os.mkdir(outputfolder + '/' + 'sorted_bowtie')
	sorted_bowtie = outputfolder + '/' + 'sorted_bowtie'
	for metagenome in repeat_list:
		file1 = folder_pair1 + '/' + metagenome + '.bowtie'
		file2 = folder_pair2 + '/' + metagenome + '.bowtie'
		fileout= sorted_bowtie + '/' + metagenome + '.bowtie'
		with open(fileout, 'w') as stdout:
...
for rep in repeat_list: 
    for data in import_text(sorted_bowtie + '/' + rep + '.bowtie', '\t'):
	readid[data[0]] = ''
for rep in repeat_list: 
    for data in import_text(sorted_bowtie + '/' + rep + '.bowtie', '\t'):	
...
def convert(x):
    ...
    repname = ""
    for i in x:
        repname = repname + '/' + rev_repeat_key[int(i)] 
...
if allcountmethod  == "TRUE":
	...
	fout1 = open(outputfolder + '/' + outputfile_prefix + '_class_fraction_counts.txt' , 'w')
	for key in classfractionalcounts.keys():
	    print >> fout1, str(key) + '\t' + str(classfractionalcounts[key])  
	fout2 = open(outputfolder + '/' + outputfile_prefix + '_family_fraction_counts.txt' , 'w')
	for key in familyfractionalcounts.keys():
	    print >> fout2, str(key) + '\t' + str(familyfractionalcounts[key])
	fout3 = open(outputfolder + '/' + outputfile_prefix + '_fraction_counts.txt' , 'w')
	for key in fractionalcounts.keys():
...
import os
import subprocess
if os.path.exists(outputfolder + '/' + outputfile_prefix + '_regionsorter.txt'):
	command = shlex.split('rm ' + outputfolder + '/' + outputfile_prefix + '_regionsorter.txt')
	p = subprocess.Popen(command)
	p.communicate()
if os.path.exists(outputfolder + '/' + 'pair1_bowtie'):
	command = shlex.split('rm -r ' + outputfolder + '/' + 'pair1_bowtie')
	p = subprocess.Popen(command)
	p.communicate()
if os.path.exists(outputfolder + '/' + 'pair2_bowtie'):
	command = shlex.split('rm -r ' + outputfolder + '/' + 'pair2_bowtie')
	p = subprocess.Popen(command)
	p.communicate()
if os.path.exists(outputfolder + '/' + 'sorted_bowtie'):
	command = shlex.split('rm -r ' + outputfolder + '/' + 'sorted_bowtie')
	p = subprocess.Popen(command)
	p.communicate()
print "... Done"
